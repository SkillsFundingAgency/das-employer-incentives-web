...
{
    [TestFixture]
    public class WhenBankDetailsAvailableIsConfirmed
    {
        ...
        public async Task Then_a_reminder_email_is_sent_to_the_user_so_they_can_rejoin_the_bank_details_journey()
        {
            ...
            model.CanProvideBankDetails = true;
            _applicationService.Setup(x => x.GetApplicationLegalEntity(_accountId, _applicationId)).ReturnsAsync(_accountLegalEntityId);
            _emailService.Setup(x => x.SendBankDetailsReminderEmail(It.Is<SendBankDetailsEmailRequest>(x => x.EmailAddress == _emailAddress))).Returns(Task.CompletedTask);
            ...
            result.ActionName.Should().Be("AddBankDetails");

            _emailService.Verify(x => x.SendBankDetailsReminderEmail(It.Is<SendBankDetailsEmailRequest>(x => x.EmailAddress == _emailAddress)), Times.Once);
        }
    }
}
